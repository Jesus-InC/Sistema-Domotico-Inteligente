    ...
    <section class="card" id="manual-card">
      ...
      <div class="card" style="margin-top:10px; padding:10px;">
        <div class="card-header">
          ...
          <div class="card-sub">Control por comandos hablados (futuro)</div>
        </div>
        <label class="switch">
    ...
    <section class="card">
      ...
      <div class="card-header">
        <div>
          ...
          <div class="card-sub">Wi-Fi + Notificaciones Telegram (futuro)</div>
        </div>
      </div>
      ...
      <div class="two-col" style="margin-bottom:8px;">
        ...
        <div class="card" style="padding:10px;">
          ...
          <div class="form-row">
            ...
            <input id="wifi-ssid" placeholder="Nombre de la red" disabled>
          </div>
          <div class="form-row" style="margin-top:4px;">
            <label for="wifi-pass">Contraseña</label>
            <input id="wifi-pass" type="password" placeholder="********" disabled>
          </div>
        ...
        <div class="card" style="padding:10px;">
          ...
          <div class="form-row">
            ...
            <input id="tg-token" placeholder="123456:ABC-DEF..." disabled>
          </div>
          <div class="form-row" style="margin-top:4px;">
            <label for="tg-chat">Chat ID</label>
            <input id="tg-chat" placeholder="123456789" disabled>
          </div>
          <label class="switch" style="margin-top:6px;">
            <input type="checkbox" id="tg-enabled" disabled>
            <span class="switch-slider"></span>
...
<script>
  ...
  // ----------------------------
  // Aplicar estado desde JSON
  // ----------------------------
  function applyState(s) {
    if (!s) return;

    // estados ON/OFF
    document.getElementById("foco-status").textContent   = (s.foco && s.foco != 0) ? "ON" : "OFF";
    document.getElementById("bomba-status").textContent  = (s.bomba && s.bomba != 0) ? "ON" : "OFF";
    document.getElementById("vent-status").textContent   = (s.vent && s.vent != 0) ? "ON" : "OFF";

    const vel = s.vel ?? 0;
    document.getElementById("vent-slider").value = vel;
    updateVentLabel(vel);

    // modo (0=manual,1=auto,2=smart)
    let modeStr = "manual";
    if (s.modo === 1) modeStr = "auto";
    else if (s.modo === 2) modeStr = "smart";
    applyModeUI(modeStr);
    ...
    if (s.hum === undefined) {
      ...
      document.getElementById("hum-val").textContent = s.hum.toFixed(1) + " %";
    }

    if (s.suelo === undefined) {
      document.getElementById("soil-val").textContent = "-- %";
    } else {
      document.getElementById("soil-val").textContent = s.suelo + " %";
    }

    document.getElementById("temp-extra").textContent = "Setpoint: 25 °C";
    document.getElementById("soil-extra").textContent = "Setpoint: 30 %";
  ...
  function sendCommand(device, action) {
    ...
    if (device === "foco") {
      url = (action === "ON") ? "/api/foco/on" : "/api/foco/off";
    } else if (device === "bomba") {
      url = (action === "ON") ? "/api/bomba/on" : "/api/bomba/off";
    } else if (device === "vent") {
      url = (action === "ON") ? "/api/vent/on" : "/api/vent/off";
    }

    if (!url) {
      console.error("Comando desconocido", device, action);
      return;
    }
  ...
  function setVentSpeed(val) {
    ...
    fetch(`${API_BASE}/api/vent/vel?v=${v}`)
      .then(r => r.json())
      .then(s => {
        applyState(s);
      })
  ...
  function setMode(mode) {
    let url = null;
    if (mode === "manual") url = "/api/mode/manual";
    else if (mode === "auto") url = "/api/mode/auto";
    else if (mode === "smart") url = "/api/mode/smart";
  ...
  // ----------------------------
  // ASISTENTE DE VOZ (placeholder)
  // ----------------------------
  function toggleVoice(enabled) {
    // Aún no está implementado en el ESP32
    document.getElementById("voice-label").textContent = enabled ? "Activado" : "Desactivado";
    alert("Asistente de voz aún no implementado en el firmware 😊");
  }

  // ----------------------------
  // WiFi / Telegram (placeholder)
  // ----------------------------
  function saveWiFi() {
    alert("Configuración Wi-Fi desde la UI todavía no está implementada en el firmware.");
  }

  function forgetWiFi() {
    alert("Olvidar Wi-Fi todavía no está implementado en el firmware.");
  }

  function saveTelegram() {
    alert("Configurar Telegram desde la UI todavía no está implementado en el firmware.");
  }
